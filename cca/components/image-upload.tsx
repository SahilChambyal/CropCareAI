...
const ImageUpload = ({disabled, onChange, onRemove, value}: ImageUploadProps) => {
    ...
    return ( <div>
        ...
        <>
        <div className="mb-4 flex items-center gap-4 z-[1]">
            {value.map(url => (
                    <div className="relative w-52 h-52 rounded-md overflow-hidden z-[1] " key={url}>
                        <Image 
                            fill
        ...
        :(<div className="w-52 h-52 rounded-md overflow-hidden border border-dashed border-gray-800 flex items-center justify-center flex-col gap-3">
            {isLoading ? (<>
                ...
                <label >
                    ...
                    <input type="file" 
                    onChange={onUpload}
                    accept="image/*" className="w-0 h-0"/>
                </label>
            </>)}
        </div>)}
